        ...
        <ul className="space-y-1">
          {items.map(({ id, title }, i) => (
            <li key={id}>
              <button
                onClick={() => scrollTo(id)}
                className={`text-left w-full text-xs px-3 py-1.5 rounded-lg transition-all duration-200 ${
                  activeId === id
                    ? "bg-primary/10 text-primary font-medium border border-primary/20"
                    : "text-muted-foreground hover:text-foreground hover:bg-muted/20"
                }`}
              >
                <span className="mr-1.5 text-[10px] text-muted-foreground/50">{i + 1}.</span>
                {title}
              </button>
            </li>
          ))}
