...
export function ArchitectureOverview({ imageSrc, title = "Architecture Diagram", caption }: ArchitectureOverviewProps) {
  const [isFullscreen, setIsFullscreen] = useState(false);

  return (
    <>
      <div
        className="group relative rounded-2xl border border-border/50 bg-card/60 backdrop-blur-xl overflow-hidden transition-all duration-400 hover:border-primary/30 cursor-pointer"
      ...
      >
        ...
        <div className="p-6 md:p-8 group-hover:scale-[1.01] transition-transform duration-500 origin-center">
          ...
          <div className="flex justify-center rounded-xl bg-muted/30 border border-border/30 p-4 md:p-6 overflow-hidden">
            <img
              src={imageSrc}
              alt={title}
              className="w-full max-w-[900px] h-auto rounded-lg object-contain"
              loading="lazy"
            />
          </div>
          {caption && (
            <p className="mt-4 text-sm text-muted-foreground text-center italic">{caption}</p>
          )}
        </div>
      </div>
      ...
      <AnimatePresence>
        {isFullscreen && (
          <>
            ...
            >
              <div
                className="relative w-full max-w-5xl max-h-full overflow-auto rounded-2xl border border-border/50 bg-card/95 backdrop-blur-xl p-8 md:p-12"
                style={{ boxShadow: "0 0 80px hsla(250, 80%, 65%, 0.15)" }}
              >
                <button
                  onClick={() => setIsFullscreen(false)}
                ...
                {caption && (
                  <p className="mt-6 text-sm text-muted-foreground text-center italic">{caption}</p>
                )}
              </div>
            </motion.div>
          </>
