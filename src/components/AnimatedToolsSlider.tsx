...
export function AnimatedToolsSlider() {
  ...
  return (
    <section ref={sectionRef} className="py-28 overflow-hidden">
      ...
      {/* Marquee Container — fixed height so it never overlaps */}
            ...
            {marqueeTools.map((tool, index) => (
              <div key={`m-${tool.name}-${index}`} className="flex-shrink-0 flex flex-col items-center gap-3 group cursor-pointer">
                <img
                  src={tool.logo}
                  alt={tool.name}
                  className="w-14 h-14 md:w-[72px] md:h-[72px] object-contain transition-transform duration-300 group-hover:scale-110 drop-shadow-lg"
                />
      ...
      {/* Stat Cards — separate container with explicit top margin */}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 md:gap-8">
          {stats.map((stat, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative p-6 md:p-8 rounded-2xl bg-card/60 backdrop-blur-xl border border-border/50 hover:border-primary/30 transition-all duration-500 hover:shadow-[0_0_30px_hsla(250,80%,65%,0.12)] hover:-translate-y-[5px]"
            >
